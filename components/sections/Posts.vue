<template>
  <div class="px-[20px] lg:px-[40px] pt-[100px] pb-[55px]" style="background-color: rgb(245, 246, 247);">
    <div>
        <div class="pt-[0px] pb-[40px] md:flex flex-wrap justify-between items-center md:text-left text-center max-w-full">
          <div>
            <div  class=" text-[14px] md:text-[15px] text-[rgb(255,131,26)] block font-epilogue font-semibold	leading-[28.8px] pb-[14.4px] mt-0 mr-0 uppercase tracking-[4px] box-border not-italic">{{ $settings.sections.posts.text }}</div>
            <div class="box-border  w-[100%]  text-[32px] lg:w-[70%] text-black block font-epilogue md:text-[37px] md:w-[72%] lg:text-[45px] font-medium  md:leading-[67.5px] m-0 p-0 no-underline pb-[14.4px]">{{ $settings.sections.posts.title }}</div>
          </div>

              <nuxt-link :to=" $settings.sections.posts.url " class="
               button-url-a
              ">{{ $settings.sections.posts.text_url }}</nuxt-link>
        </div>
    </div>
    ...
    <div class=" flex flex-wrap w-full ">
      <div v-for="(item, i) in items" :key="i" class="w-full pb-3 md:w-1/2 lg:w-1/3 sm:pb-0 p-2">
...
.button-url-a:focus, .button-url:hover {
    color: #fff;
    background-color: #FF831A;
}


.button-url-a:-webkit-any-link {
